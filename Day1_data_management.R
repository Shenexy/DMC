...
#assignment
Female <- filter(kisumu_main_vendor, gender == "Female")
manyattaWard <- filter(kisumu_main_vendor, ward == "Manyatta")
kiosk <- filter(kisumu_main_vendor, vendor_type == "Kiosk")
manyattawardandkiosk <- filter(kisumu_main_vendor, ward == "Manyatta" & vendor_type == "Kiosk")

#assignment
obungamale <- filter(kisumu_main_vendor, slum == "Obunga" & gender =="Male")
obungaFemale <- filter(kisumu_main_vendor, slum == "Obunga" & gender =="Female")
obungaFemale1 <- filter(kisumu_main_vendor, slum == "Obunga" & gender =="Multiple females")

#Example:  selects rows where the animal age is less than 12 weeks
data <- filter(Animal_Data, age < 12)

#assignment
newdata <- filter(Animal_Data, age > 10)
